...
def beli_barang(kode_barang, jumlah):
    ...
    keterangan = f"Penjualan {nama_barang} ({kode_barang}) - {jumlah} unit"
...
def neraca_saldo():
    """Membuat neraca saldo dari jurnal umum"""
    wb = load_workbook(JURNAL_FILE)
    ws = wb.active
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0]:  # Pastikan ada data
            ...
            if akun not in saldo_akun:
                saldo_akun[akun] = {'debit': 0, 'kredit': 0}
            
            saldo_akun[akun]['debit'] += debit
            saldo_akun[akun]['kredit'] += kredit
    
    # Hitung saldo bersih setiap akun
    neraca = []
    total_debit_saldo = 0
    total_kredit_saldo = 0
    
    for akun, saldo in saldo_akun.items():
        saldo_debit = saldo['debit'] - saldo['kredit']
        saldo_kredit = saldo['kredit'] - saldo['debit']
        
        if saldo_debit > 0:
            neraca.append((akun, saldo_debit, 0))
            total_debit_saldo += saldo_debit
        elif saldo_kredit > 0:
            neraca.append((akun, 0, saldo_kredit))
            total_kredit_saldo += saldo_kredit
        else:
            neraca.append((akun, 0, 0))
    
    return neraca, total_debit_saldo, total_kredit_saldo
...
class Aplikasi:
    ...
    def admin_menu(self):
        ...
        buat_tombol("Neraca Saldo", self.tampilkan_neraca)
    ...
    def tampilkan_neraca(self):
        win = Toplevel(self.root)
        win.title("Neraca Saldo")
        win.geometry("800x600")
        
        data, total_debit, total_kredit = neraca_saldo()
        
        tree = ttk.Treeview(win, columns=("Akun", "Debit", "Kredit"), show='headings')
        tree.heading("Akun", text="Nama Akun")
        tree.heading("Debit", text="Saldo Debit")
        tree.heading("Kredit", text="Saldo Kredit")
        
        # Atur lebar kolom
        tree.column("Akun", width=300)
        tree.column("Debit", width=200, anchor=E)
        tree.column("Kredit", width=200, anchor=E)
        
        for row in data:
            debit_str = f"Rp{row[1]:,.2f}" if row[1] > 0 else ""
            kredit_str = f"Rp{row[2]:,.2f}" if row[2] > 0 else ""
            tree.insert('', END, values=(row[0], debit_str, kredit_str))
        ...
        Label(frame_total, text=f"Total Saldo Debit: Rp{total_debit:,.2f}", 
              font=('Arial', 12, 'bold')).pack(side=LEFT, padx=20)
        Label(frame_total, text=f"Total Saldo Kredit: Rp{total_kredit:,.2f}", 
              font=('Arial', 12, 'bold')).pack(side=LEFT, padx=20)
    ...
    def form_beli_barang(self):
        ...
        Label(win, text="Nama Barang").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        kode = Entry(win, width=25)
        kode.grid(row=0, column=1, padx=10, pady=5)
